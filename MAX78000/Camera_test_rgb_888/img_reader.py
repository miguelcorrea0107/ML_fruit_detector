...
def parse_image_rgb888(img, x, y):
    ...
    for i in range(0, len(img), 4):
        ...
        pixels[idx, 2] = b
        idx += 1
    # Reshape the pixels array to match the image dimensions
    pixels = pixels.reshape((x, y, 3))
    return pixels

ser = serial.Serial('/dev/tty.usbmodem14402', 115200, timeout=30)  # open serial port
